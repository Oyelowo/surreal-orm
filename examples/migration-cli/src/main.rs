...
use surreal_orm::migrator::{MigrationConfig, RollbackStrategy};
...
async fn main() {
    ...
    migration_cli(db, Resources).await;
}
...
async fn migration_cli(db: Surreal<impl Connection>, codebase_resources: Resources) {
    // let db = initialize_db().await;
    let cli = Cli::parse();

...
async fn test_create_alien_with_id_not_specified_but_generated_by_the_database(
    ...
    let created_ship = create(space_ship.clone()).get_one(db.clone()).await?;
...
async fn test_create_alien_with_links() -> SurrealdbOrmResult<()> {
    ...
    let created_spaceship1 = create(space_ship.clone()).get_one(db.clone()).await?;
    let created_spaceship2 = create(space_ship2.clone()).get_one(db.clone()).await?;
    let created_spaceship3 = create(space_ship3.clone()).get_one(db.clone()).await?;
...
async fn test_alien_build_output() -> SurrealdbOrmResult<()> {
    ...
    let created_spaceship1 = create(space_ship1.clone()).get_one(db.clone()).await?;
    let created_spaceship2 = create(space_ship2.clone()).get_one(db.clone()).await?;
    let created_spaceship3 = create(space_ship3.clone()).get_one(db.clone()).await?;
    let point = point! {
        x: 40.02f64,
...
async fn test_create_set_statement() -> SurrealdbOrmResult<()> {
    ...
    let _space_ship2 = create::<SpaceShip>(vec![
    ...
    let _space_ship3 = create::<SpaceShip>(vec![

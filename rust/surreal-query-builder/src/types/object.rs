    ...
    ($struct_name:ident { $($key:ident: $value:expr),* $(,)? }) => {
        {
            ...
            type __StructNameRenamedFields = <$struct_name as $crate::SurrealModel>::StructRenamedCreator;
...
macro_rules! object_partial {
    ($struct_name:ident { $($key:ident: $value:expr),* $(,)? }) => {
        {
            $crate::check_unique_idents!($($key), *);
            type __StructNameRenamedFields = <$struct_name as $crate::SurrealModel>::NonNullUpdater;
            $crate::validators::assert_fields!(__StructNameRenamedFields : $( $key ),*);

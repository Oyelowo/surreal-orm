...
struct Account {
    ...
    #[surreal(one_to_one = "Account")]
    friend: Account,

    #[surreal(one_to_one = "Account")]
    favorite_project: Foreign<Project>,
...
// use account::schema::model as account_model;
use account::schema::Account as AccountSchema;
...
impl Account {
    const fn get_schema() -> account::schema::Account<0> {
...
mod project {
    ...
    use super::ReleaseSchema as Release;
    use surreal_simple_querybuilder::prelude::*;
...
// use project::schema::Project as ProjectSchema;
type ProjectSchema<const N: usize> = project::schema::Project<N>;

impl Project {
    type Schema = project::schema::Project<0>;
    const fn get_schema() -> ProjectSchema<0> {
        // project::schema::model
        //  account::schema::Account<0>::new()
        project::schema::Project::<0>::new()
    }
}
...
use release::schema::Release as ReleaseSchema;

impl Release {
    type Schema = release::schema::Release<0>;
    const fn get_schema() -> Self::Schema {
        release::schema::model
    }
}
...
fn test_create_account_query() {
    // account.friend().
    // let x = Account::get_schema().projects().authors().projects().authors().projects();
    // let x: Account::Schema = Account::schema;
    // project_model.authors().friend().email
    // ProjectSchema::new().authors().friend().email
    // Account::get_schema().projects().authors.
    Project::get_schema()
        .authors()
        .projects()
        .authors()
        .projects()
        .authors();
    // Account::get_schema().projects().authors().email.
    let query = QueryBuilder::new()
        ...
        .set_model(&Account::get_schema()) // Can also use get_schema
        .unwrap()
        .build();

...
export const INGRESS_EXTERNAL_PORT_LOCAL = await getPort({
	port: portNumbers(8080, 8200),
});
...
export const ingressHostsLocal = [
	"localhost",
	baseHostLocal,
	...subs.map((d) => `${d}.${baseHostLocal}`),
];
...
export function getIngressUrlHost({
	environment,
	subDomain,
}: Prop): IngressHost {

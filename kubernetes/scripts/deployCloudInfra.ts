...
export const Argv = yargs(process.argv.slice(2))
	.options({
		"skip-preview": {
			...
			boolean: true,
			describe:
				"Skip showing preview. Useful for CI(Continuous Integration) pipeline.",
			// demandOption: true,
		},
		LINODE_TOKEN: {
			alias: "lt",
...
function main() {
	...
	try {
		handleShellError(
			sh.rm("-rf", `${path.join(baseDir, `Pulumi.${environment}.yaml`)}`),
		);
		handleShellError(
			sh.exec(
				`export PULUMI_CONFIG_PASSPHRASE='not-needed' && pulumi stack init --stack ${environment}`,
			),
		);
	} catch {

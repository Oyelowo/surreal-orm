...
export const getSecretsSample = () => {
	...
	} satisfies TResourcesEnvVars satisfies Record<
		TResourceCategory,
		Partial<Record<ResourceName, any>>
	>;
};
		...
		_.mapValues(getSecretsSample(), (rCategory) => {
			return z.object(
				_.mapValues(rCategory, (resourceName) => {
					return z.object(
						_.mapValues(resourceName, (_secret) => string) as any,
					);
				}),
			);
		}),
...
const ENVIRONMENTS_ALL: Environment[] = [
	"test",
	"local",
	"development",
	"staging",
...
> {
	constructor(
		private resourceCat: TCat,
		private resourceName: TResource,
		private environment: Environment,
	) {}

	getSecrets = (): TSecretsKubeManifests[TCat][TResource] => {
		const allSecretsJson = PlainSecretsManager.#getSecretJsonObject(
			this.environment,
		);
		return parseSecrets(allSecretsJson, { requireValues: false })[
			this.resourceCat
		][this.resourceName];
	};
	...
	static syncAll = (): void => {
		ENVIRONMENTS_ALL.forEach((environment) => {
			...
			if (_.isEmpty(existingEnvSecret)) {
				sh.touch(envPath);
			}
			...
			const mergedObject = parseSecrets(
				R.mergeDeepLeft(existingEnvSecret, getSecretsSample()),
				{
					requireValues: false,
				},
			);
	...
	static #getSecretJsonObject = (
		environment: Environment,
	): object | undefined => {
		const envPath = PlainSecretsManager.#getSecretPath(environment);

		const existingEnvSecret = this.#parseJson<object>(
			sh.exec(`cat ${envPath}`, { silent: true }).stdout.trim(),
		);

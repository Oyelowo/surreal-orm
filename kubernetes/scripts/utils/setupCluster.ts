...
function applySetupManifests(kubeObject: KubeObject) {
	...
	sh.exec(
		`kubectl rollout status deployment/${sealedSecretsName} -n=${namespaces.kubeSystem}`,
	);
	...
	sh.exec(
		`kubectl rollout status deployment/${certManager.chart} -n=${namespaces.certManager}`,
	);
	sh.exec(
		`kubectl rollout status deployment/${certManagerTrust.chart} -n=${namespaces.certManager}`,
	);
	...
	if (kubeObject.getEnvironment() !== "local") {
		...
		sh.exec(
			"kubectl -n argocd rollout restart deployment argocd-argo-cd-server",
		);

		applyResourceManifests(
			"infrastructure/argocd-applications-parents",
			kubeObject,
		);
	}
}
...
function applyResourceManifests(
	resourcePath: ResourceOutputDirProps["outputDirectory"],

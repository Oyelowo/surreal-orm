...
export function syncEtcHostsWithCustomHosts() {
	...
	if (hostsFileContent.stderr) {
		console.error(
			chalk.blueBright(
				`Something went wrong. Error: ${hostsFileContent.stderr}`,
			),
		);
	}
	...
	function updateIpHost(customHost: string) {
		const found = existingIpHosts.find(
			([existingIp, existingHost]) =>
				existingIp === LOCAL_IP && existingHost === customHost,
		);
		if (found) {
			return;
		}
		...
		console.info(
			chalk.blueBright(`Updating Ip and host: ${LOCAL_IP} - ${customHost}`),
		);

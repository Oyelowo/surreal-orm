...
const switchToCluster = (name: string) => {
	const selectContext = sh.exec(`kubectl config use-context ${name}`, {
		silent: true,
	});
...
async function createCluster(
	clusterChoices: string[],
	likelyLocalCluster: (s: string) => boolean,
) {
	...
	const answerNewCluster = await inquirer.prompt<
		Record<typeof newClusterNameKey, boolean>
	>([
		{
			type: "input",
			name: newClusterNameKey,
			message: chalk.blueBright(
				"What do you want to name your cluster? Press Enter to use default",
			),
	...
	sh.exec(
		`minikube start -p  ${answerNewCluster.newClusterName} --kubernetes-version=v1.24.0`,
	);
...
export async function promptKubernetesClusterSwitch(environment: Environment) {
	...
	const clusterChoicesWithSeparators = clusterChoices.flatMap((context) => [
		context,

...
*/
    syncAppSealedSecrets = async (allManifestsInfo: KubeObjectInfo[]) => {
        const selectedUnsealedSecretsInfo = await prompSecretResourcesSelection(this.environment, allManifestsInfo);
        ...
        for (let unsealedSecret of selectedUnsealedSecretsInfo) {
            this.mergeUnsealedSecretToSealedSecret({
    ...
    mergeUnsealedSecretToSealedSecret = ({ sealedSecretInfo, unsealedSecretInfo }: MergeProps): void => {
...
class SealedSecretsManagerPrompter {
    constructor() {
...
they want to update
*/
    prompSecretResourcesSelection = async (allManifestsInfo: KubeObjectInfo[]): Promise<KubeObjectInfo[]> => {
        ...
        const appSecretKeysByNamespace = await this.#promptSecretKeysSelection(selectedSecretResources);

        return this.#filterSecrets(selectedSecretResources, appSecretKeysByNamespace);
    }


    #promptSecretKeysSelection = async (allSecretResources: KubeObjectInfo[]): Promise<AppSecretKeysByNamespace> => {

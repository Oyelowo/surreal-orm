...
export function mergeUnsealedSecretToSealedSecret(props: Props): void {
	for (const secret of props.secretKubeObjects) {
		...
		const sealedSecretDir = p.join(
			secret.resourceBaseDir,
		...
		const sealedSecretFilePath = p.join(
			sealedSecretDir,
			`sealed-secret-${name}-${namespace}.yaml`,
		);
		sh.exec(
			`echo '${yaml.stringify(updatedSealedSecret)}' > ${sealedSecretFilePath}`,
		);
	}
}
...
function updateExistingSealedSecret({
	...
	const existingSealedSecretJsonData = existingSealedSecretKubeObjects?.find(
		matchesUnsealedSecret,
	);

	// Pick only selected secrets for encytption
	const filteredSecretData = _.pickBy(
		secretData,
		(_v, k): boolean => !!selectedSecretsForUpdate?.includes(k),
	);
	...
	const mergedEncryptedData = _.pickBy(encryptedData, (_v, k) =>
		unfilteredSecretKeys.includes(k),
	);

...
type AppSecretKeysWithinNamespaces = Record<
	Namespace,
	Record<ResourceName | string, SecretKey[]>
>;
...
export async function selectSecretKubeObjectsFromPrompt(
	...
	const secretKubeObjectsSorted = _.sortBy(secretKubeObjects, [
		(s) => s.metadata.namespace !== "applications",
	]);
	const sercretObjectsByNamespace = _.groupBy(
		secretKubeObjectsSorted,
		(s) => s.metadata.namespace,
	);

	// Name and value have to be defined for inquirer if not using basic string
	const mapToPrompterValues = (
		secret: TKubeObject,
	): { name: string; value: TKubeObject } => ({
    ...
    Namespace  ===> infra
         ...
         - infra 2
     */
	const applicationList = Object.entries(sercretObjectsByNamespace).flatMap(
		([namespace, namespaceSecretObjects]) => [
	...
	const secretkeysData = await promptSecretObjectDataSelection(
		promptResponse.selectedSecretObjects,
	);
...
async function promptSecretObjectDataSelection(
	...
	const createAppSecretDataSelectionPrompt = (
		resource: TKubeObject<"Secret">,
	) => {
		...
		return {
			...
			message: chalk.cyanBright(
				`Select secrets from  ${name} in the ${namespace} namespace`,
			),
			choices: secretKeys.flatMap((secretKey) => [
				new inquirer.Separator(),
				secretKey,
			]),
			pageSize: 2000,

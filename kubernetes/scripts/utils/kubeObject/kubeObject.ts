...
export type TKubeObjectBaseCommonProps<K extends ResourceKind> =
	KubeObjectSchema & {
...
export type TKubeObject<K extends ResourceKind = ResourceKind> = Extract<
	KubeObjectCustom,
	{ kind: K }
>;
...
export class KubeObject {
	...
	getForApp = (
		...
		return this.kubeObjectsAll.filter((m) =>
			m.path.startsWith(`${resourceDir}${path.sep}`),
		);
	};
	...
	private syncAll = (): this => {
		...
		manifestsPaths.forEach((manifestPath, i) => {
			...
			const kubeObject = yaml.parse(
				fs.readFileSync(manifestPath, "utf8"),
			) as TKubeObject;
			if (_.isEmpty(kubeObject)) {
				throw new Error(
					chalk.redBright`Manifest is empty. Check the directory that all is well`,
				);
			}
			...
			if (kubeObject.kind === "Secret") {
				const encodedStringData = _.mapValues(
					kubeObject.stringData,
					(v): string => Buffer.from(v ?? "").toString("base64"),
				);

				kubeObject.data = R.mergeDeepRight(
					kubeObject.data ?? {},
					encodedStringData,
				);
			}
	...
	private getManifestsPathWithinDir = (
	...
	getOfAKind = <K extends ResourceKind>(kind: K): TKubeObject<K>[] => {
		return this.kubeObjectsAll.filter(
			(o) => o.kind === kind,
		) as TKubeObject<K>[];
	};
...
*/
	syncSealedSecrets = (): void => {
		const secrets: TKubeObject<"Secret">[] = this.getOfAKind("Secret").map(
			(s) => {
				return {
					...s,
					// Syncs all secrets
...
*/
	syncSealedSecretsWithPrompt = async (): Promise<void> => {
		const selectedSecretObjects = await selectSecretKubeObjectsFromPrompt(
			this.getOfAKind("Secret"),
		);
		mergeUnsealedSecretToSealedSecret({
			existingSealedSecretKubeObjects: this.getOfAKind("SealedSecret"),
	...
	sealSecretValue({
		...
		name,
		secretValue,
	}: { namespace: string; name: string; secretValue: string }) {

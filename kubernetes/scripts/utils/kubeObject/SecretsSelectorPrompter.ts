...
export class SecretKubeObjectPrompter {
    constructor() { }
     ...
     *  Namespace  ===> application
        ...
        Namespace  ===> infra
             ...
             - infra 2
     **/
    static selectFromPrompt = async (secretKubeObjects: TSecretKubeObject[]): Promise<TSecretKubeObject[]> => {
        // We want applications secrets first
        const secretKubeObjectsSorted = _.sortBy(secretKubeObjects, [
        ...
        const { selectedSecretResources: selectedSecretObjects } = await inquirer.prompt<{ [allResourceAnswerKeyName]: TSecretKubeObject[]; }>({
            type: 'checkbox',
        ...
        const selectedSecretsDataKeys = await SecretKubeObjectPrompter.#promptSecretDataSelection(selectedSecretObjects);

        return SecretKubeObjectPrompter.#pickBySelectedSecretDataKeys(selectedSecretObjects, selectedSecretsDataKeys);
    };
    ...
    * "Select secrets from oyelowo-app in the application namespace"
     ...
     * ...
     */
    static #promptSecretDataSelection = async (secretObjectsInfo: TSecretKubeObject[]): Promise<AppSecretKeysWithinNamespaces> => {
        ...
        return await inquirer.prompt<AppSecretKeysWithinNamespaces>(secretObjectsInfo.map(createAppSecretDataSelectionPrompt));
    };
     ...
     * ...
     * */
    static #pickBySelectedSecretDataKeys = (
        ...
        return secretObjects?.map((info) => {
            ...
            let filteredSecretRecords = _.pickBy(secretData, (_v, k) => selectedSecretDataKeys.includes(k));
